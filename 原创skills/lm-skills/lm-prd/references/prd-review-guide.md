...
| 6 | | 编号规范 | 所有 `##` 统一渲染为 `<h2>`？需求点（`###`）渲染为 `<h3>`？编号与 spec 一致（整篇持续编号，见 spec-template.md）？ |
...
完整项（deep-check 时追加）：prd-guide.md 不可更改项（15条）+ prototype-guide.md 全部规则。

### D. 侧边栏与导航

| # | ★ | 维度 | 检查点 |
|---|---|------|--------|
| 18 | | sidebar-title | sidebar-header 中有需求名称（`.sidebar-title`）？与 spec `# 标题` 一致？ |
| 19 | | hero-badge | hero 区无 `.hero-badge`？ |
| 20 | ★ | 导航层级 | 一级导航可点击（`<a>` + `href`，非 `<div>`）？hover 有视觉反馈？二/三级导航缩进合理？最多三级？ |
...
**11 项**（PRD 侧）：A1 需求List（含列顺序）、A3 位置和触发场景、A4 核心逻辑、A5 需求点顺序（含proto-embed位置）、B8 section-title使用、B10 原型覆盖、B12 内容边界、B13 布局规范、C14 CSS模板、C15 .changed、D20 导航层级
