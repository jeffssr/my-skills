...
2. **用户提供图片时读取**：`references/image-insert-guide.md`
...
用户提供图片且要求插入时，读取 `references/image-insert-guide.md`：
- URL 图片 → 直接写入 spec `**原型**：截图` 下方
- 本地图片 → PicGo 上传后写入 URL
...
spec 中 `**原型**：截图` 的需求点，proto-embed 区用 `<img>` 替代手绘 HTML。详见 `references/image-insert-guide.md`。
